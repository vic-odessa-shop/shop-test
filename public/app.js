...
function renderProducts(items) {
    ...
    items.forEach(p => {
        ...
        card.innerHTML = `
            <div class="info-icon" onclick="showDescModal('${encodeURIComponent(p.name)}','${encodeURIComponent(p.desc || '')}','${encodeURIComponent(p.image || '')}')">i</div>
...
/* ================== МОДАЛКА ОПИСАНИЯ ================== */
function showDescModal(nameEnc, descEnc, imgEnc) {
    const modal = document.getElementById('desc-modal');
    document.getElementById('desc-text').innerText = decodeURIComponent(descEnc || 'Смачна позиція нашого меню');
